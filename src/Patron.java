	...
	public void run() {
		try {
			...
			for(int i=0;i<lengthOfOrder;i++) {
				...
				if (i == 0) {
                    firstDrink = System.currentTimeMillis(); // Record the first drink time for the patron
                    System.out.println("First drink: " + firstDrink);
		
				}
				totalExecutionTime=totalExecutionTime+drinksOrder[i].getExecutionTime();
			}
			

			endTime = System.currentTimeMillis();
			long totalTime = endTime - startTime; //from when order is taken until job finishes
			long waitingTime=(totalTime)-totalExecutionTime;
		    long responseTime = firstDrink-arrivalTime;
